...
class Menu extends MY_Controller
{
    ...
    {
        $id = $this->session->userdata('status') == 'Pimpinan' ? ['1', '2'] : '1';
        $data = [
            'title' => 'Menu Access',
            'role' => $this->db->where_not_in('id', $id)->get('tb_user_role')->result_array(),
        ];
    ...
    public function delete($id)
    {
        ...
        $delete = $this->db->where('id_um', $id)->delete('tb_user_menu');
        if ($delete) {
            $this->session->set_flashdata('message', "<div class='alert alert-success' role='alert'> Selamat, data berhasil dihapus.. </div>");
        } else {
            $this->session->set_flashdata('message', "<div class='alert alert-danger' role='alert'> Opps.., data gagal dihapus </div>");
        }
